...
namespace SanyaPlugin
{
        ...
        IEventHandlerUpdate
    {
        ...
        //ScoreDB
        public List<PlayerScoreInfo> scoredb = new List<PlayerScoreInfo>();

        //-----------------------Event---------------------
        public EventHandler(SanyaPlugin plugin)
        {
            this.plugin = plugin;
            SanyaPlugin.eventhandler = this;
        }
        ...
        public void OnRoundStart(RoundStartEvent ev)
        {
            ...
            switch(eventmode)
            {
                ...
                case SANYA_GAME_MODE.CLASSD_INSURGENCY:
                    ...
                    break;
                case SANYA_GAME_MODE.BREED_939:
                    scp939_startscpamount = RoundSummary.singleton.GetStartClassList().scps_except_zombies;
                    break;
            }
        }
            ...
            if(roundduring)
            {
                ...
                if(plugin.score_summary_inround)
                {
                    ...
                    scoredb.Sort((a, b) =>
                    {
                        ...
                        int kill = scp != 0 ? scp : b.killamount - a.killamount;
                        return kill != 0 ? kill : b.damageamount - a.damageamount;
                    });
                    string bcstr = "ラウンド終了！\n";
                ...
                if(plugin.summary_less_mode)
                {
                    ...
                    plugin.Info($"[SummaryLess] Started Restarttime:{restarttime}s");
                    plugin.Server.Map.ClearBroadcasts();
                    plugin.Server.Map.Broadcast((uint)restarttime, $"<size=30>ラウンドが終了しました。\n{restarttime}秒後にリスタートします。</size>", false);

                    Type summary = RoundSummary.singleton.GetType();
        ...
        public void OnRoundRestart(RoundRestartEvent ev)
        {
            ...
            scp939_floorlist.Clear();
            scp939_killcount = 0;
            scp939_mode = 0;
            scp939_startscpamount = 0;
        ...
        public void OnPlayerJoin(PlayerJoinEvent ev)
        {
            ...
            if(plugin.score_summary_inround)
            {
                if(scoredb.Find(x => x.player.PlayerId == ev.Player.PlayerId) == null)
                {
                    plugin.Debug($"[ScoreDB/Add] {ev.Player.Name}");
                    scoredb.Add(new PlayerScoreInfo(ev.Player));
                }
            }
        }
        ...
        {
            if(ev.IP == "localClient") return;
            plugin.Debug($"[OnPlayerLeave] {ev.Name}/{ev.PlayerID}/{ev.SteamID}/{ev.IP}");

            //ScoreDB
            if(plugin.score_summary_inround)
            {
                PlayerScoreInfo target = scoredb.Find(x => x.player.PlayerId == ev.PlayerID);
                if(target != null)
                {
                    plugin.Debug($"[ScoreDB/Remove] {ev.Name}");
                    scoredb.Remove(target);
                }
            }
        }
            ...
            if(!ev.Cancel && AlphaWarheadController.host.inProgress)
            {
                ...
                if(plugin.cassie_subtitle && roundduring)
                {
                    ...
                    if(!ev.IsResumed)
                    {
                        ...
                        {
                            if(plugin.nuke_subtitle_name_hide && ev.Activator.TeamRole.Team != Smod2.API.Team.NINETAILFOX && ev.Activator.TeamRole.Team != Smod2.API.Team.SCIENTIST && ev.Activator.TeamRole.Role != Role.CLASSD)
                            {
                                plugin.Server.Map.Broadcast(15, plugin.alphawarhead_countdown_start_message.Replace("[name]", "Unknown").Replace("[team]", "Unknown").Replace("[second]", count.ToString()), false);
                    ...
                    else
                    {
                        ...
                        if(ev.Activator != null)
                        {
                            if(plugin.nuke_subtitle_name_hide && ev.Activator.TeamRole.Team != Smod2.API.Team.NINETAILFOX && ev.Activator.TeamRole.Team != Smod2.API.Team.SCIENTIST && ev.Activator.TeamRole.Role != Role.CLASSD)
                            {
                                plugin.Server.Map.Broadcast(10, plugin.alphawarhead_countdown_resume_message.Replace("[name]", "Unknown").Replace("[team]", "Unknown").Replace("[second]", count.ToString()), false);
            ...
            if(!ev.Cancel)
            {
                ...
                if(plugin.cassie_subtitle && roundduring)
                {
                    ...
                    if(ev.Activator != null)
                    {
                        if(plugin.nuke_subtitle_name_hide && ev.Activator.TeamRole.Team != Smod2.API.Team.NINETAILFOX && ev.Activator.TeamRole.Team != Smod2.API.Team.SCIENTIST && ev.Activator.TeamRole.Role != Role.CLASSD)
                        {
                            plugin.Server.Map.Broadcast(7, plugin.alphawarhead_countdown_stop_message.Replace("[name]", "Unknown").Replace("[team]", "Unknown"), false);
                        }
                        else
                        {
        ...
        public void OnSetRole(PlayerSetRoleEvent ev)
        {
            ...
            if(RoundSummary.RoundInProgress())
            {
                switch(eventmode)
                {
                    case SANYA_GAME_MODE.STORY_173:
                        ...
                        if(ev.Role == Role.TUTORIAL)
                        {
                            ...
                            else
                            {
                                ...
                                if(scpqueue.Count != 0)
                                {
                                    ev.Role = scpqueue[UnityEngine.Random.Range(0, scpqueue.Count)];
                    ...
                    case SANYA_GAME_MODE.CLASSD_INSURGENCY:
                        ...
                        if(roomUpstairs != null && ev.Role == Role.SCIENTIST && plugin.classd_ins_scientist_ez_spawn)
                        {
                            Timing.RunCoroutine(SanyaPlugin._DelayedTeleport(ev.Player, roomUpstairs, false), Segment.Update);
                        }
                        break;
                    case SANYA_GAME_MODE.HCZ:
                        Room room = hcz_hallways[UnityEngine.Random.Range(0, hcz_hallways.Count)];
            ...
            switch(ev.Role)
            {
                ...
                case Role.NTF_CADET:
                    ...
                    break;
                case Role.TUTORIAL:
                    targetammo = plugin.default_ammo_tutorial;
                    break;
            }
        ...
        public void OnPlayerHurt(PlayerHurtEvent ev)
        {
            ...
            {
                int pers = UnityEngine.Random.Range(0, 100);
            ...
            //LureIntercom
            if(plugin.scp106_lure_intercom_time > 0 && ev.DamageType == DamageType.LURE)
            {
                plugin.Debug($"[LureIntercom/Start] {ev.Player.Name}");
                if(!UnityEngine.Object.FindObjectOfType<LureSubjectContainer>().allowContain)
                {
                    ev.Damage = 0f;
                    ev.Player.SetGodmode(true);
                    (ev.Player.GetGameObject() as GameObject).GetComponent<Inventory>().Clear();
                    Timing.RunCoroutine(SanyaPlugin._LureIntercom(ev.Player, plugin.scp106_lure_intercom_time), Segment.Update);
                }
            }
            ...
            if(plugin.score_summary_inround && ev.Attacker.TeamRole.Team != Smod2.API.Team.SCP && ev.Player.PlayerId != ev.Attacker.PlayerId && ev.Player.TeamRole.Role != Role.TUTORIAL && ev.DamageType != DamageType.RAGDOLLLESS)
            {
                ...
                if(attacker != null)
                {
                    attacker.damageamount += (int)ev.Damage;
                }
            }
        ...
        public void OnPlayerDie(PlayerDeathEvent ev)
        {
            ...
            //----------------SCP-106コンテナの死体削除--------------
            if(ev.DamageTypeVar == DamageType.LURE)
            {
                ev.SpawnRagdoll = false;
                (ev.Player.GetGameObject() as GameObject).GetComponent<Inventory>().Clear();
            }
            ...
            {
                case SANYA_GAME_MODE.BREED_939:
                    if((ev.Killer.TeamRole.Role == Role.SCP_939_53 || ev.Killer.TeamRole.Role == Role.SCP_939_89)
                        ...
                        && ev.Player.TeamRole.Team != Smod2.API.Team.TUTORIAL)
                    {
                        ...
                        int addamount = 0;
                        int addamount3 = 0;
                        if(scp939_startscpamount > 2)
                        {
                            addamount = plugin.breed939_mode_over3_add_amount * (scp939_startscpamount - 2);
                            addamount3 = addamount * 2;
                        }

                        if(scp939_killcount >= plugin.breed939_mode1_amount + addamount && scp939_mode == 0)
                        {
                            scp939_mode++;
                            plugin.Info($"[Eventer:BREED_939] ModeChange 0 -> 1");

                            if(plugin.cassie_subtitle)
                            {
                                plugin.Server.Map.ClearBroadcasts();
                                plugin.Server.Map.Broadcast((uint)10, plugin.breed939_modechange_message, false);
                            }
                            ...
                            if(spectators.Count > 0)
                            {
                                ...
                                targetspec.ChangeRole(target939 != 0 ? Role.SCP_939_53 : Role.SCP_939_89, true, false);
                                Timing.RunCoroutine(SanyaPlugin._DelayedSetHealth(targetspec, ev.Killer.TeamRole.MaxHP / 2), Segment.Update);
                            }
                        }
                        else if(scp939_killcount >= plugin.breed939_mode2_amount + addamount && scp939_mode == 1)
                        {
                            scp939_mode++;
                            plugin.Info($"[Eventer:BREED_939] ModeChange 1 -> 2");
                        ...
                        else if(scp939_killcount >= plugin.breed939_mode3_amount + addamount3 && scp939_mode == 2)
                        {
                            scp939_mode++;
                            plugin.Info($"[Eventer:BREED_939] ModeChange 2 -> 3");
                            ...
                            Timing.RunCoroutine(SanyaPlugin._939AlwaysSee(), Segment.Update);
                        }
                    }
                    break;
            }
        ...
        {
            plugin.Debug($"[OnLure] {ev.Player.Name}:{ev.AllowContain}");

            if(plugin.scp106_lure_open_time > 0f && plugin.scp106_lure_open_time == -1 && !ev.Player.GetGodmode())
            {
                plugin.Debug($"[_DelayedOpen] time:{plugin.scp106_lure_open_time}");
                Timing.RunCoroutine(SanyaPlugin._DelayedOpenLureContainer(plugin.scp106_lure_open_time), Segment.Update);
            }
        }
        ...
        public void OnDoorAccess(PlayerDoorAccessEvent ev)
        {
            ...
            switch(eventmode)
            {
                ...
                case SANYA_GAME_MODE.HCZ:
                    ...
                    else if(eventmode != SANYA_GAME_MODE.HCZ && ev.Player.TeamRole.Role == Role.CLASSD)
                    {
                        if(startwait > plugin.Round.Duration)
                        {
                            plugin.Debug($"[Eventer]Too fast ClassD at {eventmode} : {startwait} > {plugin.Round.Duration}");
                            ev.Allow = false;
                        }
                    }
                    else if(eventmode == SANYA_GAME_MODE.HCZ && ev.Player.TeamRole.Team != Smod2.API.Team.SCP)
                    {
                        foreach(Smod2.API.Item i in ev.Player.GetInventory())
                        {
                    ...
                    break;
                case SANYA_GAME_MODE.BREED_939:
                    if(scp939_mode >= 2 && ev.Player.TeamRole.Team == Smod2.API.Team.SCP && !ev.Door.Locked)
                    {
                        ev.Allow = true;
                    }
                    break;
                    ...
                    if(comp1 == null)
                    {
                        ...
                        if(comp2 != null && comp3 != null && comp4 != null && comp5 != null && item.gameObject != null)
                        {
                            ...
                            else if(ev.KnobSetting == KnobSetting.ONE_TO_ONE && !isAfterChange && player.TeamRole.Role != Role.SCP_049_2)
                            {
                                ...
                                if(speclist.Count > 0)
                                {
                                    int targetspec = UnityEngine.Random.Range(0, speclist.Count);
                            ...
                            else if(ev.KnobSetting == KnobSetting.FINE)
                            {
                                ...
                                plugin.Server.Map.SpawnItem((ItemType)UnityEngine.Random.Range(0, 30), newpos, new Vector(0, 0, 0));
                            }
        ...
        public void OnGeneratorFinish(GeneratorFinishEvent ev)
        {
            ...
            if(engcount >= plugin.night_generators_recov_amount)
            {
                genrecovery = true;
            }
        ...
        public void On079Door(Player079DoorEvent ev)
        {
            ...
            {
                SanyaPlugin.CallAmbientSound(UnityEngine.Random.Range(0, 32));
            }
        }
        ...
        {
            plugin.Debug($"[OnHandcuffed] {ev.Player.Name}<{ev.Player.TeamRole.Role}>:{ev.Owner.Name}<{ev.Owner.TeamRole.Role}>:{ev.Allow}");

            if(plugin.cassie_subtitle)
            {
                ev.Player.PersonalClearBroadcasts();
                ev.Player.PersonalBroadcast(5, plugin.handcuffed_message, false);
            }
        }
        ...
        public void OnUpdate(UpdateEvent ev)
        {
            ...
            if(updatecounter % 60 == 0 && running && roundduring)
            {
                    ...
                    try
                    {
                        foreach(Player scp049 in plugin.Server.GetPlayers(Role.SCP_049))
                        {
                            foreach(Player scp0492 in plugin.Server.GetPlayers(Role.SCP_049_2))
                            {
                                ...
                                if(curHealth >= scp0492.TeamRole.MaxHP / 2) continue;
                                ...
                                if(plugin.scp049_healing_to_049_2_range > distance)
                                {
                                    ...
                                    scp0492.SetHealth(Mathf.Clamp(curHealth + plugin.scp049_healing_to_049_2_amount, 0, scp0492.TeamRole.MaxHP / 2), DamageType.NONE);
                                }
                            }
                        }
                    }
                ...
                {
                    int scpcount = plugin.Server.GetPlayers(Smod2.API.Team.SCP).Count;
                    if(scpcount > 0 || !plugin.outsidezone_termination_check_scp)
                    {
                        Timing.RunCoroutine(SanyaPlugin._AirSupportBomb(5, 10, true, true, true), Segment.Update);
                    }
                    else
                    {
                        plugin.Warn($"[AirBomb] Skipped... SCPs:{scpcount}");
                    }
                    airbomb_used = true;
                }
                ...
                if(plugin.outsidezone_termination_time > 0 && plugin.Round.Duration >= plugin.outsidezone_termination_time && !airbomb_used)
                {
                    ...
                    {
                        plugin.Warn($"[AirBomb] Skipped... SCPs:{scpcount}/Check:{plugin.outsidezone_termination_check_scp}");
                    }
                    airbomb_used = true;
                }
                        ...
                        foreach(Player ply in plugin.Server.GetPlayers())
                        {
                                ...
                                ply.TeamRole.Role == Role.CHAOS_INSURGENCY) && ply.IsHandcuffed())
                            {
                                    ...
                                    (pos.z >= 25 && pos.z <= 34))
                                {
                                    ...
                                    {
                                        int rndresult = UnityEngine.Random.Range(0, 100);
            ...
            if(plugin.user_command_enabled)
            {
                ...
                if(ev.ReturnMessage == "Command not found.")
                {
                                        ...
                                        if((ev.Player.GetGameObject() as GameObject).GetComponent<FallDamage>().isGrounded)
                                        {
                                            ...
                                            if(plugin.Server.Round.Duration > plugin.scp106_portal_to_human_wait)
                                            {
                                                if(health > usehp)
                                                {
                                                    if(humanlist.Count > 0)
                                                    {
                                                        int rndres = UnityEngine.Random.Range(0, humanlist.Count);
                                                        ev.Player.PersonalClearBroadcasts();
                                                        ev.Player.PersonalBroadcast(3, plugin.user_command_boost_success.Replace("[role]", ev.Player.TeamRole.Name).Replace("[hp]", $"-{usehp}"), false);
                                    ...
                                    if(gameObject != null)
                                    {
                                        ...
                                        else if(camera.cameraName.Contains("SCP-079 MAIN CAM"))
                                        {
                                            if(NineTailedFoxAnnouncer.singleton.isFree)
                                            {
                                                ...
                                                {
                                                    var target = without079[UnityEngine.Random.Range(0, without079.Count)];
                    ...
                    else if(ev.Command.StartsWith("test"))
                    {
                        ...
                        //plugin.Warn($"[ScoreDB] Show Start");
                        //foreach(PlayerScoreInfo ply in scoredb)
                        //{
                        //    plugin.Debug($"{ply.player.Name}[Kill:{ply.killamount}/Death:{ply.deathamount}/Damage:{ply.damageamount}");
                        //}
                        //plugin.Warn($"[ScoreDB] Show End");

                        //var oos = UnityEngine.Object.FindObjectOfType<OneOhSixContainer>();
                        //if(!oos.used) SanyaPlugin.Call106Scream();
        ...
        {
            plugin.Debug($"[OnShoot] {ev.Player.Name}[{ev.SourcePosition}({ev.Direction})] -> {ev.Target?.Name}{ev.TargetPosition}({ev.TargetHitbox}) [{ev.Weapon.WeaponType}:{ev.WeaponSound}] [{ev.ShouldSpawnHitmarker}:{ev.ShouldSpawnBloodDecal}]");
